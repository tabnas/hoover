...
- Hoover depends on the jsonic grammar's `val` rule. Register it on a
  grammar-bearing instance (`jsonic.Make().UseDefaults(Hoover, …)`); the
  plugin inspects `j.RSM()` up front and returns a clear `error` if `val`
  is missing or empty, instead of silently creating an empty rule. (Go's
  `Empty()` keeps the rule key with no alternates, so the guard checks
  `len(val.Open)`, not just key presence.)
