...
// Package main builds the C-ABI shared library: libtabnashoover.
//
// tabnas-clib-template: v3 (stamped by admin tasks/adopt-clib.sh;
// edit the template and re-stamp, not this file).
//
//	go build -buildmode=c-shared -o libtabnashoover.so ./clib
//
// hoover TEXT IN, VERDICT (AND VALUE) OUT. This is one of the
...
// tabnas_grammar builds a hoover parser and returns a handle to it.
...
// tabnas_parse checks one input against the hoover grammar.
//
//export tabnas_parse
