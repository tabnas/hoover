...
#
# Only this crate's entry is asserted. The sibling crates' entries (the
# engine and the support runner) legitimately move whenever those
# checkouts do, which is the same reason blanket `--locked` is wrong here.
...
# exemption: the recorded versions of the sibling path crates, the engine
# (`tabnas`) and the fixture runner (`tabnas-support`). Those entries
...
#
# Cargo.lock records the engine and the support runner by version, and
# both are resolved from sibling checkouts of MAIN. So the day either
# bumps its crate version, `--locked` here fails with "cannot update the
# lock file" on every pull request in this repo, including ones that
# touch no Rust at all -- a red build caused by another repository's
# release. Reproduced by bumping the sibling's version and re-running:
# `--locked` fails, the plain build succeeds. The engine repo has no path
# dependency of its own, which is why `--locked` is right there and wrong
# here.
